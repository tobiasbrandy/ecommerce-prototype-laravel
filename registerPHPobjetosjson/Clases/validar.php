...
class Validar {
  ...
  public function validarEditarUsuario($miUsuario, Usuario $usuarioViejo) {
    $errores = [];
    ...
    if (!password_verify($miUsuario["oldpass"], $usuarioViejo->getPassword())) {
      $errores[] = "Contraseña antigua incorrecta.";
    }
    if ($miUsuario["pass"] != $miUsuario["cpass"])
    {
        $errores[] = "Pass y Cpass son distintas";
    ...
    if ($miUsuario["mail"] != $usuarioViejo->getMail())
    {
